...
import { ArrowLeft, BarChart, Eye, MousePointerClick } from "lucide-react";
               ...
               return (
                 <div key={ad.id} className={`neomorph-raised rounded-3xl p-6 flex flex-col ${ad.status === 'active' ? 'border-2 border-primary' : ''}`}>
                   <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${ad.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {ad.status}
                        </span>
