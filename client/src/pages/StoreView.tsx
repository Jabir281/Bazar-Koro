      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-300">
          <div className="flex items-center gap-4">
             <button onClick={() => navigate("/dashboard")} className="p-3 bg-[#e8eaf0] neomorph-raised rounded-full text-[#707d40]">
                <ArrowLeft className="w-5 h-5" />
             </button>
             <div>
                <h1 className="text-3xl font-extrabold tracking-tight">{store.name}</h1>
                <p className="text-[#646657] font-medium text-sm">Owner: {store.ownerName} • {store.location.city}</p>
             </div>
          </div>
          
          <button 
             onClick={() => setShowAddProduct(!showAddProduct)}
             className="flex items-center gap-2 px-5 py-3 rounded-xl bg-[#707d40] text-white neomorph-raised active:neomorph-inset transition-all font-semibold"
          >
        ...
        {showAddProduct && (
           <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
              <div className="bg-[#e8eaf0] neomorph-raised rounded-[2rem] w-full max-w-lg p-8 relative max-h-[90vh] overflow-y-auto">
                 ...
                 <form onSubmit={handleAddProduct} className="space-y-4 text-left">
                    <div>
                      <label className="block text-xs font-bold text-[#646657] uppercase tracking-widest mb-1 pl-1">Product Name</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#646657] uppercase tracking-widest mb-1 pl-1">Description</label>
                      <div className="neomorph-inset rounded-xl p-1">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#646657] uppercase tracking-widest mb-1 pl-1">Price (Taka/USD)</label>
                      <div className="neomorph-inset rounded-xl p-1">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#646657] uppercase tracking-widest mb-1 pl-1">Image Image (Limit 2MB)</label>
                      <div 
                         onClick={() => fileInputRef.current?.click()}
                         className={`neomorph-inset rounded-xl p-8 border-2 border-dashed ${pImageBase64 ? 'border-[#707d40]' : 'border-slate-300'} flex flex-col items-center justify-center cursor-pointer hover:bg-slate-200/50 transition-colors`}
                      >
                         {pImageBase64 ? (
                    ...
                    <button type="submit" disabled={pLoading} className="w-full mt-4 bg-[#707d40] text-white py-3 rounded-xl font-bold neomorph-raised hover:scale-[1.02] active:scale-95 transition-transform disabled:opacity-50">
        ...
        {products.length === 0 ? (
           <div className="neomorph-inset rounded-3xl p-12 text-center text-[#646657] font-medium border border-slate-300 border-dashed">
              ...
              {products.map(p => (
                 <div key={p.id} className="neomorph-raised rounded-2xl p-4 flex flex-col group hover:-translate-y-1 transition-transform">
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-200">
                       <div className="text-lg font-extrabold text-[#707d40]">${p.price.toFixed(2)}</div>
                    </div>
                 </div>
              ))}
           </div>
